/**
 * TOOLFUL TYPING MASTER - COMPLETE SCRIPT
 * Includes Game Logic, Popup System, and Live Notifications
 */
...
function loadParagraph() {
    ...
    // Focus input when user clicks on typing area
...
window.onload = () => {
    loadParagraph();
    // Only focus if popup isn't visible
    if (!localStorage.getItem("toolfulUserName")) {
        inputField.blur();
    } else {
        inputField.focus();
    }
...
document.addEventListener("click", () => {
    if(!resultOverlay.classList.contains("show") && !document.getElementById("welcomePopup").style.display === "none") {
        inputField.focus();
    }
});
